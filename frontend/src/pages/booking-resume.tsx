...
import { fetchUserDataAndLocationInfo } from "../utils/summaryData"
import { Resume } from "../components/Resume/SummaryInfo"
...
export const FullSummary = () => {
  ...
  const fetchDiagnosticData = async () => {
    const diagnostic = JSON.parse(
      await storage.getItem("selectedDiagnostic")
    )
    ...
    if (itemType === "doctors") {
      fetchDoctorData().then(doctor => {
        fetchUserDataAndLocationInfo(storage).then(data => {
          if (data.error) {
            showPopup({
              message: "Sorry, some data is missing!"
            }).then(() => navigate(-1))
          }
              ...
              <ResumeBlock title="Your Diagnostic">
                <div className="resume__diagnostics">
                  ...
                  <div className="resume__diagnostics__info">
                    ...
                    <div className="resume__block__text">
                                            Price: $
                      {diagnosticData.price.toFixed(0)}
                    </div>
                  </div>
                </div>
              </ResumeBlock>
